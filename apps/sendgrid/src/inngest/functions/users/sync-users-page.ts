...
export const syncUsersPage = inngest.createFunction(
  ...
  async ({ event, step, logger }) => {
    const { organisationId, syncStartedAt, offset, region } = event.data as SyncUsersEventType;
    ...
    const nextPage = await step.run('list-users', async () => {
      ...
      const result = await getUsers(token, offset);
    ...
    if (nextPage) {
      await step.sendEvent('sync-users-page', {
        ...
        data: {
          ...(event.data as SyncUsersEventType),
          offset: nextPage,
        },
      });
      return {
