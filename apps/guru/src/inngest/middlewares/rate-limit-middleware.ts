...
import { GuruError } from '@/connectors/commons/error';
 ...
 * The middleware checks for specific errors (instances of GuruError) that indicate a rate limit has been reached,
 ...
 * - Handles GuruError, specifically looking for a 'Retry-After' header in the error response.
      ...
      onFunctionRun: ({ fn }) => {
        return {
          transformOutput: (ctx) => {
            ...
            const retryAfter =
              error instanceof GuruError && error.response?.headers.get('Retry-After');
            ...
            return {
              ...
              result: {
                ...result,
                error: new RetryAfterError(`Guru rate limit reached by '${fn.name}'`, retryAfter, {
                  cause: error,
                }),
              },
            };
